...
### Real gate Wi-Fi outage exposes cached status — 10 September 2026

The owner disabled Wi-Fi on the stationary gate. At 11:13:53.983 UTC, GetOperatorOverview still returned Closed/code 2. Five additional successful requests from 11:14:21.023 through 11:15:22.348 UTC also returned Closed, spanning 88 seconds from the first received sample. The owner-reported disable time was not independently timestamped. The response fields were operatorSerialNumber, operatorStatus, powerSupplyStatus, closingBeamStatus, openingBeamStatus, theftAlarmState and notificationFlags; no explicit online flag or measurement timestamp was supplied.

An independent non-actuating MQTT diagnostic connected to the broker and published only connectionRequest. No controller response, identity reply or live overview arrived; it timed out after 25.182 seconds. No identity, time-sync or activation packet was sent. Apple Home continued displaying Closed throughout the observation window.

This fails the real offline-availability check: successful HTTP receipt refreshes the local snapshot even when the gate is disconnected and the cloud's physical state may be cached. It corrects any interpretation that the earlier injected transport-failure test established controller-offline detection. The next implementation needs independently confirmed controller availability or live telemetry freshness before exposing an available HomeKit state. Command activation already requires a new live MQTT handshake, but that does not fix idle display availability. Restore gate Wi-Fi and verify read-only recovery before another hardware check; physical outage recovery is pending.

### Recovery and bounded live availability — alpha.6

After the owner restored gate Wi-Fi, the first read-only session received a valid status-1 identity reply but timed out waiting for overview telemetry. A follow-up received a 68-byte overview at 12.595 seconds and returned Closed with no activation. This confirms live recovery, while retaining evidence of intermittent telemetry delays.

Alpha.6 requires independent live MQTT verification before HomeKit state is available. HTTPS reads preserve that verification time rather than renewing it. Read-only verification is renewed after 20 seconds; a separate 45-second expiry pushes communication failure even while cached HTTP responses continue. Longer configured polling intervals cannot postpone verification indefinitely. A missing protocol address or unsupported region stays unavailable, and setup now asks for the protocol address even with control disabled. Monitoring and control serialize use of the account MQTT client ID: control cancels the monitor, awaits bounded teardown, then begins its own fresh session. Changed credentials or protocol addresses cannot reuse earlier verification.

New regressions first failed on the old cached-HTTP-only implementation. All 60 tests now pass, including cached Closed with a failed live probe, verification expiry through real Homebridge HAP objects, recovery, identity isolation and monitor/control teardown ordering. The earlier FirmwareRevision correction now follows alpha.6 package metadata.

A local read-only check against the gate returned Closed at 11:25:11.148 UTC after 13.006 seconds. A second HTTP read at 11:25:26.511 UTC retained the original verification time (age 15.669 seconds); a third read renewed live verification and returned Closed at 11:25:45.512 UTC after 12.999 seconds. Two live probes and zero activation packets were sent, enforced by an independent publish allowlist. This checks healthy verification renewal, not the repeated hardware outage or command handover on the installed version.
...
### Automatic recovery did not complete — alpha.6

After the owner reported restoring Wi-Fi, Apple Home still showed No Response at the first check near 11:37:40 UTC. The unchanged child bridge logged a protocol error at 11:39:21 UTC and a timeout at 11:44:46 UTC on its subsequent scheduled attempt. No restart or activation occurred. A separate HTTPS-only request at 11:41:00.569 UTC returned Closed, which remains insufficient evidence of live recovery.

Two bounded, identity-only diagnostics during the following five-minute child-bridge backoff reproduced the missing telemetry. Each received a one-byte connection reply and a non-retained 12-byte identity reply with header `[1,1,2,135]`; the strict identity decoder accepted the reply. Neither received deviceOverview before the 25-second deadline, ending at 11:46:00.553 and 11:46:45.345 UTC. An independent publish allowlist permitted only connectionRequest, cmd 01 identity and disconnect. No time-sync or activation packets were sent. These diagnostic sessions were interventions after the failed automatic-recovery observation, not evidence of unattended recovery.

A third diagnostic added the pinned Home Assistant reference's alternate `sysTpUrc` subscription. Neither telemetry topic produced a message, and it timed out at 11:47:38.367 UTC after the same accepted identity exchange. This does not support a missing alternate-topic subscription as the cause of this incident. The earlier protocol error remains unlocalized; it did not reproduce in these three probes. Possible phone-app session competition and controller telemetry behavior after reconnecting remain unresolved. The owner was asked whether either official app was still connected. No production code, configuration or process was changed during these observations.

### Live telemetry after closing official apps — alpha.6

The owner supplied a Pro screenshot reporting Wi-Fi enabled, excellent signal and Connected to Cloud: Yes, then confirmed both Pro and Remote were fully closed. The next installed-plugin observation still showed No Response, with a new protocol error at 11:55:44 UTC.

A read-only probe subsequently received a non-retained 68-byte deviceOverview and returned Closed at 11:56:27.026 UTC after 13.115 seconds. This probe also subscribed to sysTpUrc, but the successful message arrived on deviceOverview. A second probe using only the plugin's normal subscriptions returned fresh Closed at 11:56:47.890 UTC after 13.111 seconds. Both accepted the identity exchange and sent zero activation packets, with the same independent publish allowlist as the earlier diagnostics. This demonstrates live telemetry recovery after the apps were closed; it does not establish that app competition caused the earlier failures. The installed child bridge remained running and in its normal retry backoff; its visible recovery was still pending at this point.

On a later log inspection, a transport error at 11:56:14 UTC coincided with the first diagnostic connection. The assumed child-bridge backoff window was not independently verified; the separate diagnostic shares the account MQTT client ID and may have interrupted monitoring. Treat its timing as a confound, not evidence that the sessions were isolated. Future diagnostics must explicitly pause monitoring or instrument the existing session rather than infer idle windows from sparse error logs.
...
### Verified Wi-Fi address helper — alpha.7

Added an explicitly confirmed D5 Evo SMART+ / South Africa fallback taking the screenshot-confirmed Pro serial and Wi-Fi MAC. It derives one candidate using the observed full-address +2 and byte-reversal relationship, then returns it only after accepted MQTT identity authentication and non-retained live telemetry. No target, time sync or activation is supplied. Failure leaves the address unchanged; saving is separate and successful verification disables control consent. Edits during verification cannot apply a stale result, and subsequent edits invalidate a helper-generated address.

The UI and updated runtime share a filesystem session lock, including teardown, to prevent competing sessions within one installation. It rejects busy attempts without queueing; a forked-process regression checks exclusion and release after failure. This does not coordinate old runtimes, official apps, separate storage directories or standalone probes. Restart the child bridge after upgrading before using the helper. Monitoring may briefly become unavailable or back off if setup holds the lock.

All 72 tests, TypeScript compilation and formatting checks pass. Automated coverage includes address carry/reversal and invalid inputs, rejected identity, missing telemetry, changed accounts, unsupported regions, no candidate persistence, UI success/failure/stale results, and runtime read/control exclusion. A local browser preview inspected the expanded manual form with synthetic data. No live gate requests, movement commands or deployed configuration changes were made for this helper. The owner's iHost remains on alpha.6; the new setup flow and alpha.7 runtime lock still require live validation.

### Alpha.7 deployment and live setup verification

Installed archive SHA-256 `78190c8eb1519c60ed12216fb0d2051bedae8eda34f7d33d901d2c54f7196739` was checked on iHost before npm ran. npm added four dependencies and changed one package. The installed package reports alpha.7; the Homebridge config stayed byte-identical to its private pre-upgrade backup and the other top-level plugin versions remained unchanged. The temporary archive server was stopped.

Only the CENTSYS child bridge was restarted. Its log records successful alpha.7 startup at **12:45:52 UTC**, with the cached accessory loaded. The upgraded wizard retained the existing login. Account discovery again returned no gates and automatically opened the manual help.
...
### Intermittent identity-byte rejection — 14 September 2026

Owner-supplied diagnostic logs reported two `operation=mqtt, stage=identity, reason=identity-padding, bytes=12` failures at 07:28:12 and 07:39:00, each followed by status recovery logged as 43 seconds. This reason establishes that the reply passed the known envelope, length and decoded status-byte-1 checks, but at least one of the next three decoded bytes was nonzero. The actual byte values and controller-side reason are not present in the log. At the default polling interval, the first failure causes 30 seconds of backoff; the remaining time is consistent with previously observed roughly 13-second session/telemetry acquisition, but the log does not measure those components independently.

The zero requirement was introduced in alpha.5 from successful `[1,0,0,0]` captures. The [pinned MQTT reference](https://github.com/Lex-campbell/centsys_remote/blob/4d6daab50e4305fe2d3ea2c0d2df6e65737e1dba/custom_components/centsys_remote/api/mqtt_remote.py#L548-L553) takes the challenge from the last four bytes without requiring the preceding bytes to be zero. It does not document the meaning of those bytes.

A synthetic reply with one nonzero decoded byte reproduced the exact rejection. The regression at the `gateSession` boundary also failed with `identity-padding` before the change. The parser now treats the three unspecified bytes as opaque while preserving the 12-byte reply length, accepted headers and successful first status byte. Monitoring still requires fresh non-retained telemetry; rejected status and malformed telemetry still prevent time sync and activation. The regression exercises each byte independently, combined nonzero values and both accepted header variants. No controller traffic or movement commands were sent during this investigation. Validation on the physical controller remains pending; the change removes the identified local rejection without establishing the vendor meaning of the extra bytes.
