...
A fresh HTTPS response does not by itself prove that the controller is online: the vendor may return cached status. Local stale-read checks and transport-failure recovery are tested, but a real gate Wi-Fi outage still needs validation. Do not treat an unchanged cloud endpoint state as verified physical state during an outage.
